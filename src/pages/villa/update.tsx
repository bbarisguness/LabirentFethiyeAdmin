...
const VillaUpdate = () => {
    ...
    */
    ...
    const { data:villa,refetch } = useVilla(params.id as string,"/villas/"+params.id+"/?populate[distance_rulers][fields][0]=id&populate[distance_rulers][fields][1]=name&populate[features][fields][0]=id&populate[features][fields][1]=name&populate[categories][fields][0]=id&populate[categories][fields][1]=name");
                            ...
                            <Grid item xs={6}>
                                ...
                                <FormControl fullWidth>
                                    <Select
                                        ...
                                        //@ts-ignore
                                        renderValue={(selected:any) => selected.map((x) => {return x+", "})}
                                    >
                                        {
                                            categories &&
...
const getInitialValues = (villa:any) => {
    const newReservation = {
        ...
        "categories":villa?.attributes.categories.data.map((x:any) => {return x.attributes.name}) ?? [],
        "metaTitle":(villa?.attributes.metaTitle ?? "") as string,
        "metaDescription":(villa?.attributes.metaDescription ?? "") as string,
        "distance_rulers":villa?.attributes.distance_rulers.data.map((x:any) => {return x.attributes.name}) ?? [],
        "video":(villa?.attributes.video ?? "") as string,
        "features":villa?.attributes.features.data.map((x:any) => {return x.attributes.name}) ?? []
    };
