...
const DistanceRulerUpdate = (drId: any) => {
  ...
  useEffect(() => {
    if (drId.drId.drId > 0) {
      apiRequest('GET', `/distance-rulers/${drId.drId.drId}`).then((res) => {
        formik.values.name = res.data.data.attributes.name;
      });
    }
  }, [drId]);
...
export default DistanceRulerUpdate;
